...
drop_fun <- function(X, Y, drop_rate = 0.5, up_rate = 10) {
  ...
  drop.ind <- 1 * (drop.gaussian > 0) # ifelse(drop.gaussian > 0, 1, 0)
...
lasso_select_drop <- function(X, Y, X_valid, Y_valid, X_all, Y_all, lambda_lst, alpha = 0.5) {
  min.lambda <- NULL
  min.coef <- NULL
  min.mse <- Inf
  fit_result <- glmnet::glmnet(
    X, Y, lambda = lambda_lst, intercept = FALSE, 
  ...
  fit_result_all <- glmnet::glmnet(
    X_all, Y_all, lambda = min.lambda, intercept = FALSE,
    alpha = alpha, standardize = FALSE)
...
loc.feature.selection <- function(
  ...
  ################ cut by cosine ################
  cos_lst <- unlist(lapply(1:ncol(X_full), function(j) {
